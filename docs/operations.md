...
Git is the source of truth and PV data lives on NFS, so the only state that
needed a backup was the state that *points at* everything else: etcd. Since
2026-08-21 both clusters' etcd are snapshotted nightly onto the Unraid share.
Design and rationale: [docs/okf/platform/backups.md](okf/platform/backups.md).

| What | How | Where on `10.0.0.12:/mnt/user/kube_storage` | Keeps | Status |
|---|---|---|---|---|
| `vcluster-media` etcd | CronJob `vcluster-media/vcluster-media-etcd-snapshot`, 03:45 UTC, `etcdctl snapshot save` | `backups/vcluster-media-etcd/vcluster-media-etcd-<UTC>.db` | 14 days | live |
| `the-cluster` etcd | CronJob `talos-backup/talos-etcd-snapshot`, 03:15 UTC, `talosctl etcd snapshot` | `backups/the-cluster-etcd/the-cluster-etcd-<UTC>.snapshot` | 14 days | **disabled** until `matchbox/talos-machineconfigs/all.yaml` is applied, then flip `platform-backups-talos.enabled` |
| This repo, the Kratix state repo | GitHub | — | — | continuous |
| Secrets | 1Password (ExternalSecrets re-create them) | — | — | continuous |
| PV contents | NFS itself; no second copy | `nfs-provisioner-*` directories | — | accepted SPOF |
| Terraform state | PostgreSQL backend | — | — | per apply |

Alerts (`platform.backups`, warning tier): `EtcdSnapshotStale` (>36h without a
success), `EtcdSnapshotNeverSucceeded`, `HostEtcdBackupMissing` (the standing
nag while the Talos job is disabled). A run that errors is upstream `KubeJobFailed`.

### Take a snapshot now

```bash
# tenant
kubectl -n vcluster-media create job --from=cronjob/vcluster-media-etcd-snapshot manual-$(date +%Y%m%d%H%M)
kubectl -n vcluster-media logs -f job/manual-$(date +%Y%m%d%H%M) -c finalize   # prints the file written and what was pruned

# host (once enabled)
kubectl -n talos-backup create job --from=cronjob/talos-etcd-snapshot manual-$(date +%Y%m%d%H%M)
```

### Restore vcluster-media etcd

Use this when the tenant's etcd is corrupt or its data PVCs are lost but the
host cluster is fine. The restore writes a snapshot into each of the three
member PVCs with `etcdutl snapshot restore`, then lets the StatefulSet come
back. **Everything inside the vcluster returns to the moment of the snapshot**;
the syncer then reconciles host-side pods to match.

**1. Pause the automation, top down.** The vcluster StatefulSets are owned by
`Application/vcluster-vcluster-media`, which is owned by
`kratix-state-reconciler`, owned by `kratix-the-cluster`, generated by the
`bootstrap-kratix` ApplicationSet — and every level self-heals. Pausing only the
bottom one gets reverted within a minute. Do all four, in this order, and
confirm the replicas stay at zero for a few minutes before touching data:

```bash
kubectl -n argocd patch applicationset bootstrap-kratix --type merge \
  -p '{"spec":{"syncPolicy":{"applicationsSync":"create-only"}}}'
argocd app set kratix-the-cluster        --sync-policy none
argocd app set kratix-state-reconciler   --sync-policy none
argocd app set vcluster-vcluster-media   --sync-policy none
```

**2. Stop the writers.**

```bash
kubectl -n vcluster-media scale statefulset vcluster-media      --replicas 0
kubectl -n vcluster-media scale statefulset vcluster-media-etcd --replicas 0
kubectl -n vcluster-media wait --for=delete pod -l app=vcluster-etcd --timeout=5m
```

**3. Restore each member.** Pick the snapshot (`ls /mnt/user/kube_storage/backups/vcluster-media-etcd/` on Unraid), then run this Job once per member, `N` = 0, 1, 2. It wipes the member's data directory, restores the snapshot into it with the member's own name and the same `--initial-cluster` the StatefulSet uses, and exits.

```bash
SNAP=vcluster-media-etcd-2026-08-22T0345Z.db   # the file to restore
for N in 0 1 2; do kubectl apply -f - <<EOF
apiVersion: batch/v1
kind: Job
metadata:
  name: etcd-restore-$N
    ...
    spec:
      ...
      initContainers:
        - name: wipe
          ...
          command: [/bin/sh, -euc, 'rm -rf /data/member /data/restore; ls -la /data']
        ...
        - name: restore
          ...
          args:
            ...
            - restore
            - /backups/$SNAP
            - --name=vcluster-media-etcd-$N
            - --initial-cluster=vcluster-media-etcd-0=https://vcluster-media-etcd-0.vcluster-media-etcd-headless.vcluster-media:2380,vcluster-media-etcd-1=https://vcluster-media-etcd-1.vcluster-media-etcd-headless.vcluster-media:2380,vcluster-media-etcd-2=https://vcluster-media-etcd-2.vcluster-media-etcd-headless.vcluster-media:2380
            - --initial-cluster-token=vcluster-media
            - --initial-advertise-peer-urls=https://vcluster-media-etcd-$N.vcluster-media-etcd-headless.vcluster-media:2380
            - --data-dir=/data/restore
          volumeMounts:
            - { name: data, mountPath: /data }
            - { name: backups, mountPath: /backups, readOnly: true, subPath: backups/vcluster-media-etcd }
      containers:
        - name: place
          image: docker.io/alpine/k8s:1.35.6@sha256:b7a12c5ddf261994c33d2eaaa06fd69a0803ff6b38683bfa3d30a76dcdf92807
          command: [/bin/sh, -euc, 'mv /data/restore/member /data/member && rmdir /data/restore && ls -la /data /data/member']
          volumeMounts: [{ name: data, mountPath: /data }]
      volumes:
        - name: data
          persistentVolumeClaim: { claimName: data-vcluster-media-etcd-$N }
        - name: backups
          nfs: { server: 10.0.0.12, path: /mnt/user/kube_storage }
EOF
done
kubectl -n vcluster-media wait --for=condition=complete job -l job-name --timeout=10m 2>/dev/null; kubectl -n vcluster-media get jobs etcd-restore-0 etcd-restore-1 etcd-restore-2
```

The three Jobs can run in parallel — each touches only its own PVC. All three
must be `Complete` before the next step; a member left with the old data would
refuse to join a cluster restored from a different revision.

**4. Bring etcd back, then the vcluster.**

```bash
kubectl -n vcluster-media scale statefulset vcluster-media-etcd --replicas 3
kubectl -n vcluster-media rollout status statefulset vcluster-media-etcd
kubectl -n vcluster-media exec vcluster-media-etcd-0 -- etcdctl \
  --endpoints=https://localhost:2379 --cacert=/run/config/pki/etcd-ca.crt \
  --cert=/run/config/pki/etcd-server.crt --key=/run/config/pki/etcd-server.key \
  endpoint status --cluster -w table          # three members, one leader, same raft index
kubectl -n vcluster-media scale statefulset vcluster-media --replicas 3
kubectl -n vcluster-media rollout status statefulset vcluster-media
```

**5. Resume the automation, top down, and verify.** Re-enabling the
ApplicationSet is enough to cascade: it restores `kratix-the-cluster`'s policy,
which restores `kratix-state-reconciler`'s, which restores
`vcluster-vcluster-media`'s. Setting the lower ones explicitly just makes it
immediate.

```bash
kubectl -n argocd patch applicationset bootstrap-kratix --type merge \
  -p '{"spec":{"syncPolicy":{"applicationsSync":"sync"}}}'
argocd app set kratix-the-cluster      --sync-policy automated --self-heal --auto-prune
argocd app set kratix-state-reconciler --sync-policy automated --self-heal --auto-prune
argocd app set vcluster-vcluster-media --sync-policy automated --self-heal --auto-prune

kubectl --context vcluster-media get pods -A          # tenant workloads back
kubectl -n vcluster-media get pods                    # host-side synced pods reconciled
kubectl -n argocd get application vcluster-vcluster-media -o jsonpath='{.status.sync.status}/{.status.health.status}{"\n"}'
```

If the restored snapshot predates a change that the syncer had already applied
to the host, the syncer deletes the host-side pod and re-creates it from the
restored object; that is expected churn, not an error.

### Restore the-cluster etcd

Use this when the host etcd is unrecoverable (quorum lost and not coming
back, data corrupt on all three) but the nodes themselves are fine. This is
Talos's own procedure — <https://docs.siderolabs.com/kubernetes-guides/advanced-guides/disaster-recovery> —
with this cluster's addresses. It needs `talosctl` and a talosconfig, so it is
James's to run.

**1. Get the snapshot onto the workstation.**

```bash
scp root@10.0.0.12:/mnt/user/kube_storage/backups/the-cluster-etcd/the-cluster-etcd-2026-08-22T0315Z.snapshot ./db.snapshot
```

**2. Wipe etcd on every control-plane node.** `EPHEMERAL` is `/var` — etcd data,
container images, logs. Machine config (on `STATE`) survives, so the nodes come
back with the same identity and wait for a bootstrap.

```bash
talosctl -n 10.0.4.101,10.0.4.102,10.0.4.103 reset --graceful=false --reboot --system-labels-to-wipe=EPHEMERAL
# wait for them to return, then every etcd service must read "Preparing"
talosctl -n 10.0.4.101,10.0.4.102,10.0.4.103 service etcd
```

**3. Bootstrap one node from the snapshot.** The `--recover-skip-hash-check`
flag is only for a `db` file copied raw out of `/var/lib/etcd/member/snap/`; a
file produced by `talosctl etcd snapshot` carries its hash and is verified.

```bash
talosctl -n 10.0.4.101 bootstrap --recover-from=./db.snapshot
```

**4. The other two join on their own** once the control-plane endpoint
(`https://10.0.4.100:6443`) is answering. Then verify and let ArgoCD catch up:

```bash
talosctl -n 10.0.4.101,10.0.4.102,10.0.4.103 etcd members
kubectl get nodes
kubectl -n argocd get applications | grep -v 'Synced *Healthy'
kubectl get pv | grep -v Bound       # PV/PVC bindings came back with etcd; this should be empty
```

Container images were wiped with `EPHEMERAL`; every pod re-pulls, so the
first ten minutes are slow, not broken.

### What a restore does not give back

- **PV contents** — there is no second copy of the NFS data. A snapshot
  restores the objects that point at the directories, not the directories.
- **Anything written after the snapshot.** Nightly means up to 24h of loss
  inside the tenant; the host cluster is rebuilt from git on top of the
  snapshot, so its loss is only the object state git does not carry.
- **A rehearsed procedure.** Neither of these has been exercised end to end
  yet. The tenant one can be rehearsed against a throwaway vcluster request;
  do that before trusting it.
...
```bash
# 1. If the nodes survived and only etcd is gone, do NOT rebuild: follow
#    "Restore the-cluster etcd" above instead -- it brings back the PV/PVC
#    bindings, Kratix status and cluster registrations that a rebuild loses.
...
# 3. Take a fresh etcd snapshot of both clusters (see "Take a snapshot now")
kubectl -n talos-backup     create job --from=cronjob/talos-etcd-snapshot          pre-upgrade-$(date +%Y%m%d)
kubectl -n vcluster-media   create job --from=cronjob/vcluster-media-etcd-snapshot pre-upgrade-$(date +%Y%m%d)
```
...
# If the control plane is unusable after the upgrade, restore the pre-upgrade
# etcd snapshot -- see "Restore the-cluster etcd" above.
```
...
- **Backup Schedules**: the two etcd snapshot CronJobs (see Backup and Recovery)
