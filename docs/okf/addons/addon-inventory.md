...
description: Every addon defined under addons/ — enabled state, namespace, chart/version, defining layer, and purpose — as of 2026-08-20, plus the kargo/kargo-projects pair added 2026-08-21, on 2026-08-30 the removal of vcluster-coredns-config and the addition of the coredns-host-config / coredns-workload pair, and on 2026-09-17 the specmarshal addon.
tags: [addons, inventory]
status: stable
generated: { by: claude-code/claude-opus-5, at: 2026-09-17T23:59:00Z }
...
| specmarshal | specmarshal (+ Jobs in `specmarshal-agents`) | manifest (`addons/.../specmarshal/`) | Specmarshal's orchestrator; each agent iteration is a Job it `kubectl exec`s into. **Ships at `replicas: 0`**; preconditions in the addon's README.md. Added 2026-09-17 |
