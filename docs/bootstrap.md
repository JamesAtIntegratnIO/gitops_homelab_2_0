...
```mermaid
sequenceDiagram
    participant Node as 🖥️ Bare Metal Node
    participant DHCP as 📡 DHCP Server<br/>(Unraid/Router)
    participant MB as 🎯 Matchbox Server<br/>10.0.112.2
    participant Talos as 🐧 Talos Linux
    
    Note over Node: BIOS: PXE Boot Enabled
    Node->>DHCP: 1. DHCP Discover<br/>(MAC: 00:23:24:e7:29:40)
    DHCP->>Node: 2. DHCP Offer<br/>IP: 10.0.4.101<br/>Next-server: 10.0.112.2<br/>Filename: undionly.kpxe
    
    Node->>MB: 3. TFTP: Get undionly.kpxe
    MB->>Node: iPXE binary
    ...
    Node->>MB: 6. Download kernel + initrd
    MB->>Node: Talos assets
    
    Note over Node: Boot Talos kernel
    
    Node->>MB: 7. HTTP GET controlplane1.yaml
    MB->>Node: Machine config
    
    Note over Talos: Install to disk<br/>/dev/disk/by-label/TALOS_INSTALL<br/>Apply config<br/>(kubelet, etcd, certs)
    
    Note over Node: Reboot from disk
    
    Note over Talos: ✅ Talos Running<br/>etcd: 10.0.4.101:2380<br/>kube-apiserver: 10.0.4.101:6443<br/>Ready for bootstrap
```
