...
| Grafana | 13.2.0 (kiwigrid sidecar 2.10.1) | admin via 1Password, OIDC via Authentik. Already on the community chart -- kube-prometheus-stack 88.5.3 depends on `grafana-community/grafana` 12.11.1, not the deprecated `grafana/grafana` (which tops out at app 12.3.1) |
| Loki / Promtail | 3.7.6 / 3.5.1 | Loki on the community chart `grafana-community/loki` 18.11.0 -- grafana/loki 7.x is Grafana Enterprise Logs only. Promtail is on its **final** chart (6.17.1, deprecated); the software is EOL and has no community fork -- see [known issues](known-issues.md) |
